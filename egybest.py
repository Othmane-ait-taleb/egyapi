    ...
    def search(self, query, includeShows=True, includeMovies=True, originalOrder=False):
        ...
        try:
            ...
            for result in searchResults:
                ...
                if link.split("/")[3] == "movie" and includeMovies:
                    resultsList.append(Episode(link, title, posterURL, rating))
                if link.split("/")[3] == "anime" and includeShows:
                    resultsList.append(Show(link, title, posterURL, rating))
    ...
    def __getTop(self, listType, pageNum):
        ...
        try:
            ...
            for result in results:
                ...
                topList.append(
                    Episode(link, title, posterURL, rating) if listType == "movies" else Show(link, title, posterURL,
                                                                                              rating))
    ...
    def getAnimeEpisodes(self):
        try:
            ...
            for episode in episodes:
                ...
                titleClass = episode.find(attrs={"class": "ep_num"})
                episodeTitle = titleClass and titleClass.text
    ...
    def getDownloadSources(self):
        try:
            ...
            for i in range(0, len(qualityLinksArray) - 2, 2):
                ...
                mediaLink = requests.utils.quote(qualityLinksArray[i + 1], safe=":/").replace("_", "%5F").replace(
                    "/stream/", "/dl/").replace("/stream.m3u8", f"/{fileName}")
